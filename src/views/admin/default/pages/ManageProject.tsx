...
const ManageProject: React.FC = () => {
  ...
  const removeMembersFromAllMembers = () => {
    const updatedAllMembers = allMembers.filter(
      (allMember) =>
        !teamMembers.some(
          (teamMember) => teamMember.userId === allMember.userId
        )
    );
    console.log("updatedAllMembers", updatedAllMembers);
    setAllMembers(
      updatedAllMembers.filter((member) => member.userId !== managerId)
    );
  };
  ...
  const handleClickAddMemberButton = (member: User) => {
    ...
    if (
      addMember &&
      deleteMembers.some(
        (deleteMember) => deleteMember.userId === addMember.userId
      )
    ) {
      setDeleteMembers(
  ...
  const handleClickRemoveMemberButton = (member: User) => {
    ...
    if (
      member &&
      addMembers.some((addMember) => addMember.userId === member.userId)
    ) {
      setAddMembers(
        addMembers.filter((addMember) => addMember.userId !== member.userId)
      );
    } else {
